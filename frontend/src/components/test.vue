...
async function fetchData() {
  try {
    const res1 = await axios.get('http://localhost:5001/api/edges')
    edges.value = res1.data

    const res2 = await axios.get('http://localhost:5001/api/nodes')
...
async function computeACPM() {
  try {
    const res = await axios.get('http://localhost:5001/api/acpm')
...
async function computeShortestPath() {
  ...
  try {
    const response = await fetch(
      `http://localhost:5001/api/path?start_id=${startId}&end_id=${endId}`,
    )
    if (!response.ok) {
