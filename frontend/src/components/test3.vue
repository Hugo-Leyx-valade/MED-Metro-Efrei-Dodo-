    ...
    const endId = ref('')
    let pathLayer = null
    const departureTime = ref('08:00:00') // valeur par défaut
    ...
    const nodesRes = await fetch('http://localhost:5000/api/nodesV3')
    stations.value = await nodesRes.json()

    const edgesRes = await fetch('http://localhost:5000/api/edgesV3')
