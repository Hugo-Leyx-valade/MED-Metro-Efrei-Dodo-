    ...
    <div id="container">
      ...
      <aside id="sidebar">
        ...
        >
          ...
          <p><strong>⏱️ Durée :</strong> {{ totalDuration }} min</p>
          <p><strong>🌍 CO₂ :</strong> {{ co2Emission }} kg</p>
          <p>
            <strong>🚗 Avec une voiture vous auriez consomer : <br /></strong> {{ co2EmissionV }} kg
            de CO₂
          </p>
        </div>
...
const startId = ref('')
const endId = ref('')
const departureTime = ref('08:00') // Valeur par défaut
...
const co2Emission = computed(() => {
  return totalDuration.value !== null ? (0.18 * (totalDuration.value / 60)).toFixed(3) : ''
})
const co2EmissionV = computed(() => {
  return totalDuration.value !== null ? (6, 5 * (totalDuration.value / 60)).toFixed(3) : ''
})
let pathLayer = null
...
function initMap() {
  ...
  // Ajout du réseau (segments atténués)
  const segments = []
    ...
    if (from && to) {
      ...
      const segment = L.polyline(
        ...
        ],
        {
          ...
          weight: 2,
          opacity: 0.2,
          interactive: false, // ne gêne pas le clic
        },
      )
...
async function fetchPath() {
  ...
  // Création du chemin mis en avant
  const latlngs = []
  const colors = []
